...
def publish_inventory_release(data, trace_id):
    event = {"order_id": data["order_id"], "status": "RELEASED"}
...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        log_event(
            "order-service",
            trace_id,
            f"Payment failed received (retry={retry_count})",
            data,
        )
        ...
        if order.status == "FAILED":
            log_event(
                "order-service",
                trace_id,
                f"Duplicate FAILED ignored for order {order.id}",
                data,
                level="WARNING",
            )
            ch.basic_ack(delivery_tag=method.delivery_tag)
        ...
        if can_transition(order.status, "FAILED"):
            ...
            db.commit()
            log_event(
                "order-service", trace_id, f"Order {order.id} moved to FAILED", data
            )
        ...
        log_event(
            "order-service",
            trace_id,
            "Error processing payment failed event",
            {"error": str(e)},
            level="ERROR",
        )
        ...
        if retry_count < MAX_RETRIES:
            ...
            publish("payment_failed", json.loads(body), headers=new_headers)

        else:
            log_event("order-service", trace_id, "Sending to DLQ", data)

            publish("payment_failed_dlq", json.loads(body), headers=headers)
    ...
    while True:
        try:
            ...
            channel.basic_consume(
                queue="payment_failed", on_message_callback=callback, auto_ack=False
            )

            log_event(
                "order-service", "SYSTEM", "Waiting for payment_failed events...", {}
            )
