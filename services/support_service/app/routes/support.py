...
def create_ticket(ticket: schemas.TicketCreate, db: Session = Depends(get_db)):
    ...
    log_event(
        ...
        data={"ticket_id": new_ticket.id, "user_id": ticket.user_id},
    )
...
def add_message(msg: schemas.MessageCreate, db: Session = Depends(get_db)):
    ...
    ticket = (
        ...
        .filter(models.SupportTicket.id == msg.ticket_id)
        .first()
    )
    ...
    log_event(
        ...
        data={"ticket_id": msg.ticket_id, "sender_id": msg.sender_id},
    )
...
@router.get(
    "/ticket/{ticket_id}/messages", response_model=list[schemas.MessageResponse]
)
